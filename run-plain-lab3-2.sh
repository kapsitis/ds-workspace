...
#######################################################
## Run tests in lab3-2 directory.                    ##
#######################################################

# Remove GRADES file (will append to it later on)
rm -f GRADES-LAB3-2.txt
...
then
  ...
  do
    ...
    then      
      ...
      if [ $? == "0" ]
      then
        echo "Test${i}: PASS" >> ../../GRADES-LAB3-2.txt
      else
        echo "Test${i}: FAIL" >> ../../GRADES-LAB3-2.txt
      fi
    else
      echo "Test${i}: TIMEOUT" >> ../../GRADES-LAB3-2.txt
    fi        
  done
else 
  ...
  for i in "${tsts[@]}"
  do
    echo "Test${i}: ERROR" >> ../../GRADES-LAB3-2.txt
  done
fi
echo "------------------------"
